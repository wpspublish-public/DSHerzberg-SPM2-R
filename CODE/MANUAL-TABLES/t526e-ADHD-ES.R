...
# vs typ status
Home_ADHD_Clin_Stand_preMatch <- bind_rows(
  Home_ADHD_Clin_preMatch,
  Home_ADHD_Stand_preMatch 
) %>% 
...
# cond_prob_ADHD <- Home_ADHD_Clin_Stand_preMatch %>% 
...
#   cond_prob_ADHD,
#   here(
#     "INPUT-FILES/COND-PROB-ADHD/cond-prob-ADHD-inputData.csv"
#   )
# )

# matchit cannot process NA. First get sum of NA for data. If that is 0,
# proceed. If sum NA is positive, recode all NA to 999
sum(is.na(Home_ADHD_Clin_Stand_preMatch))

# identify cols with NA
na_cols <- Home_ADHD_Clin_Stand_preMatch %>% select_if(~ any(is.na(.)))

# in NA cols, replace NA with 999
Home_ADHD_Clin_Stand_preMatch <- Home_ADHD_Clin_Stand_preMatch %>%
  replace_na(
    list(
      IDNumber = 999,
      AgeInMonths = 999,
...
match <- matchit(
  ...
  data = Home_ADHD_Clin_Stand_preMatch, 
  method = "nearest", 
  ratio = 1)
match_summ <- summary(match)

# save matched samples into new df; split by clin_status
Home_ADHD_Clin_Stand_match <- match.data(match) %>% 
  select(-Group, -distance, -weights) #%>% 
Home_ADHD_Stand_match <- Home_ADHD_Clin_Stand_match %>% 
  filter(clin_status == 'typ')
Home_ADHD_Clin_match <- Home_ADHD_Clin_Stand_match %>% 
  filter(clin_status == 'clin')

# demo counts
source(here("CODE/VAR-CAT-ORDER/var-cat-order-demo-tables.R"))

match_dist_Stand <- Home_ADHD_Stand_match %>% 
...
match_dist_Clin <- Home_ADHD_Clin_match %>% 
  select(age_range, Gender, ParentHighestEducation, Ethnicity) %>% 
  gather("Variable", "Category") %>% 
  group_by(Variable, Category) %>%
  count(Variable, Category) %>%
...
match_dist_Home <- bind_rows(
  ...
  mutate(form_dx = case_when(
    rownames(.) == "1" ~ 'Home-ADHD',
    T ~ NA_character_
  )) %>% 
  select(form_dx, everything())

rm(list=setdiff(ls(), c("Home_ADHD_Stand_match", "match_dist_Home")))
...
Home_ADHD_matchStand <- Home_ADHD_Stand %>% 
  semi_join(Home_ADHD_Stand_match, by ='IDNumber')

scale_order <- c("SOC", "VIS", "HEA", "TOU", 
                 "TS", "BOD", "BAL", "PLA", "TOT")

# Write matched typical t-score descriptives
Home_ADHD_matchStand_t_desc <-
  Home_ADHD_matchStand %>% 
  select(contains('_NT')) %>% 
...
Home_ADHD_match_t_desc <- bind_cols(Home_ADHD_matchStand_t_desc,
                                   Home_ADHD_clin_t_desc) %>%
...
rm(list = setdiff(ls(), c('match_dist_Home', 'Home_ADHD_match_t_desc')))

### SCHOOL DATA ---------------------------------------------------

# READ AND COMBINE FINALIZED SAMPLES --------------------------------------------------

School_ADHD_Stand_preMatch <- 
  suppressMessages(as_tibble(read_csv(
    here("OUTPUT-FILES/COMBINED-T-SCORES-PER-CASE/School-Stand-All-T-scores-per-case.csv")
  )))


School_ADHD_Clin_preMatch <-
  suppressMessages(as_tibble(read_csv(
...
# This step encodes a logical var (Group), needed by matchit, that captures clin
# vs typ status
School_ADHD_Clin_Stand_preMatch <- bind_rows(
  School_ADHD_Clin_preMatch,
  School_ADHD_Stand_preMatch 
) %>% 
  mutate(Group = case_when(
...
# matchit cannot process NA. First get sum of NA for data. If that is 0,
# proceed. If sum NA is positive, recode all NA to 999
sum(is.na(School_ADHD_Clin_Stand_preMatch))

# identify cols with NA
na_cols <- School_ADHD_Clin_Stand_preMatch %>% select_if(~ any(is.na(.)))

# in NA cols, replace NA with 999
School_ADHD_Clin_Stand_preMatch <- School_ADHD_Clin_Stand_preMatch %>%
  replace_na(
    list(
      IDNumber = 999,
      AgeInMonths = 999,
...
match <- matchit(
  ...
  data = School_ADHD_Clin_Stand_preMatch, 
  method = "nearest", 
  ratio = 1)
match_summ <- summary(match)

# save matched samples into new df; split by clin_status
School_ADHD_Clin_Stand_match <- match.data(match) %>% 
  select(-Group, -distance, -weights) #%>% 
School_ADHD_Stand_match <- School_ADHD_Clin_Stand_match %>% 
  filter(clin_status == 'typ')
School_ADHD_Clin_match <- School_ADHD_Clin_Stand_match %>% 
  filter(clin_status == 'clin')

# demo counts
source(here("CODE/VAR-CAT-ORDER/var-cat-order-demo-tables.R"))

match_dist_Stand <- School_ADHD_Stand_match %>% 
...
match_dist_Clin <- School_ADHD_Clin_match %>% 
  select(age_range, Gender, Ethnicity) %>% 
  gather("Variable", "Category") %>% 
  group_by(Variable, Category) %>%
  count(Variable, Category) %>%
...
# Extract match cases from stand sample
School_ADHD_matchStand <- School_ADHD_Stand %>% 
  semi_join(School_ADHD_Stand_match, by ='IDNumber')

scale_order <- c("SOC", "VIS", "HEA", "TOU", 
                 "TS", "BOD", "BAL", "PLA", "TOT")

# Write matched typical t-score descriptives
School_ADHD_matchStand_t_desc <-
  School_ADHD_matchStand %>% 
  select(contains('_NT')) %>% 
  rename_all(~ str_sub(., 1, -4)) %>% 
  describe(fast = T) %>%
  rownames_to_column() %>% 
...
School_ADHD_match_t_desc <- bind_cols(School_ADHD_matchStand_t_desc,
                                     School_ADHD_clin_t_desc) %>%
  mutate(ES = abs((mean_typ - mean_clin) / ((sd_typ + sd_clin / 2))),
         form_dx = case_when(
           rownames(.) == "1" ~ 'School-ADHD',
...
rm(list = setdiff(ls(), c(
  'match_dist_Home', 'Home_ADHD_match_t_desc',
  'match_dist_School', 'School_ADHD_match_t_desc'
)))
...
write_csv(match_dist,
          here(
            str_c(
              'OUTPUT-FILES/MANUAL-TABLES/t526e-ADHD-matchDemos-',
...
ADHD_match_t_desc <- bind_rows(
  Home_ADHD_match_t_desc,
  School_ADHD_match_t_desc
)

# write table comping t-score descriptives with ES
write_csv(ADHD_match_t_desc,
          here(
            str_c(
              'OUTPUT-FILES/MANUAL-TABLES/t526e-ADHD-ES-',
              format(Sys.Date(), "%Y-%m-%d"),
              '.csv'
            )
          ),
