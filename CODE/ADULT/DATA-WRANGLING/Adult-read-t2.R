...
Adult_Self_time2 <-
  suppressMessages(as_tibble(read_csv(
    here("INPUT-FILES/ADULT/TIME-2/SPM-2 Adult ages 1690 Self-Report Questionnaire time2.csv")
  ))) %>% select(
    IDNumber,
    Age,
    Gender,
...
output_1690 <- map_dfc(score_names,
                       ~
                         Adult_Self_time2 %>% left_join(eval(as.name(
                           str_c(.x, '_1690_lookup_col')
                         )),
                         by = str_c(.x, '_raw')) %>%
                         select(!!str_c(.x, '_NT'))) %>%
  bind_cols(Adult_Self_time2, .) %>% 
  arrange(IDNumber)
...
write_csv(
  output_1690,
  here(
    'OUTPUT-FILES/ADULT/T-SCORES-PER-CASE/Adult-Self-time2-T-Scores-per-case.csv'
  ),
  na = ''
)
