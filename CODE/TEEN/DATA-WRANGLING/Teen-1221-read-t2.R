...
Teen_1221_Home_time2 <-
  suppressMessages(as_tibble(read_csv(
    here("INPUT-FILES/TEEN/TIME-2/SPM-2 Teen ages 1221 Home Report Questionnaire time2.csv")
  ))) %>% select(
    IDNumber,
    Age,
    Gender,
...
output_1221 <- map_dfc(score_names,
                     ~
                       Teen_1221_Home_time2 %>% left_join(eval(as.name(
                         str_c(.x, '_1221_lookup_col')
                       )),
                       by = str_c(.x, '_raw')) %>%
                       select(!!str_c(.x, '_NT'))) %>%
  bind_cols(Teen_1221_Home_time2, .) %>% 
  arrange(IDNumber)
...
write_csv(
  output_1221,
  here(
    'OUTPUT-FILES/TEEN/T-SCORES-PER-CASE/Teen-1221-Home-time2-T-Scores-per-case.csv'
  ),
  na = ''
)
...
Teen_1221_School_time2 <-
  suppressMessages(as_tibble(read_csv(
    here("INPUT-FILES/TEEN/TIME-2/SPM-2 Teen ages 1221 School Report Questionnaire time2.csv")
  ))) %>% select(
    IDNumber,
    Age,
    Gender,
...
output_1221 <- map_dfc(score_names,
                       ~
                         Teen_1221_School_time2 %>% left_join(eval(as.name(
                           str_c(.x, '_1221_lookup_col')
                         )),
                         by = str_c(.x, '_raw')) %>%
                         select(!!str_c(.x, '_NT'))) %>%
  bind_cols(Teen_1221_School_time2, .) %>% 
  arrange(IDNumber)
...
write_csv(
  output_1221,
  here(
    'OUTPUT-FILES/TEEN/T-SCORES-PER-CASE/Teen-1221-School-time2-T-Scores-per-case.csv'
  ),
  na = ''
)
...
Teen_1221_Self_time2 <-
  suppressMessages(as_tibble(read_csv(
    here("INPUT-FILES/TEEN/TIME-2/SPM-2 Teen ages 1221 Self-Report Questionnaire time2.csv")
  ))) %>% select(
    IDNumber,
    Age,
    Gender,
  ...
  # select(-AgeGroup) %>% 
  # Compute raw scores. Note use of `rowSums(.[TOT_items_Child_512_Self])`: when used 
  # within a pipe, you can pass a vector of column names to `base::rowSums`, but you
  # must wrap the column vector in a column-subsetting expression: `.[]`, where the
  # dot is a token for the data in the pipe.
  mutate(
...
output_1221 <- map_dfc(score_names,
                       ~
                         Teen_1221_Self_time2 %>% left_join(eval(as.name(
                           str_c(.x, '_1221_lookup_col')
                         )),
                         by = str_c(.x, '_raw')) %>%
                         select(!!str_c(.x, '_NT'))) %>%
  bind_cols(Teen_1221_Self_time2, .) %>% 
  arrange(IDNumber)
...
write_csv(
  output_1221,
  here(
    'OUTPUT-FILES/TEEN/T-SCORES-PER-CASE/Teen-1221-Self-time2-T-Scores-per-case.csv'
  ),
  na = ''
)
