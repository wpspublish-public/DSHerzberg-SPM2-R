...
Child_512_School <- Child_512_School_items #%>% 
#  select(
#    -(q0014:q0122)
#  )   
...
# Child_512_School_TOT_freq_AgeGroup <- Child_512_School %>% group_by(age_range) %>% count(TOT_raw) %>%
...
#   Child_512_School %>% group_by(age_range) %>% arrange(age_range) %>% summarise(n = n(),
...
# 
# AgeGroup <- Child_512_School_TOT_desc_AgeGroup %>% pull(age_range)

# Plot TOT_raw means, SDs by age_range
# mean_plot <- ggplot(data = Child_512_School_TOT_desc_AgeGroup, aes(group, mean)) +
#   geom_point(
#     col = "blue",
#     fill = "blue",
#     alpha = .5,
...
# Child_512_School_dup <- Child_512_School %>% count(IDNumber) %>% filter(n > 1)
# write_csv(Child_512_School_dup, here("OUTPUT-FILES/CHILD/Child_512_School_dup.csv"))
...
# TOT_nz_obj <- bestNormalize(Child_512_School$TOT_raw)
# 
...
# TOT_nz_transform <- eval(as.name(chosen_transform))(Child_512_School$TOT_raw)
...
raw_score_cols_list <- map(score_names, ~ Child_512_School %>% 
              pull(
...
# each case.
Child_512_School <- Child_512_School %>% bind_cols(NT_cols)

# write T-scores per case table to .csv
write_csv(Child_512_School, here(
  paste0(
    'OUTPUT-FILES/CHILD/T-SCORES-PER-CASE/Child-512-School-T-Scores-per-case-',
...
# MASS::truehist(Child_512_School$TOT_NT, h = 1)
# hist_plot <- ggplot(data = Child_512_School, aes(TOT_NT)) +
#   geom_histogram(
#     binwidth = .2,
#     col = "red"
#   ) +
...
TOT_lookup <- Child_512_School %>% group_by(
  TOT_raw
) %>% 
...
write_csv(all_lookup, here(
  paste0(
    'OUTPUT-FILES/CHILD/RAW-T-LOOKUP-TABLES/Child-512-School-raw-T-lookup-',
    format(Sys.Date(), "%Y-%m-%d"),
    '.csv'
  )
), 
...
write_csv(all_lookup_pub, here(
  paste0(
    'OUTPUT-FILES/CHILD/PRINT-FORMAT-NORMS-TABLES/Child-512-School-print-raw-T-lookup-',
    format(Sys.Date(), "%Y-%m-%d"),
    '.csv'
  )
), 
...
write_csv(Child_512_School_raw_desc, here(
  paste0(
    'OUTPUT-FILES/CHILD/DESCRIPTIVES/Child-512-School-raw-desc-',
    format(Sys.Date(), "%Y-%m-%d"),
    '.csv'
  )
), 
...
cat_order <- c("5 to 8 years", "9 to 12 years",
               "Male", "Female", 
               "Hispanic", "Asian", "Black", "White", "MultiRacial", "AmericanIndAlaskanNat", "Other", 
               "northeast", "midwest", "south", "west")                                   


Child_512_School_demo_counts <- Child_512_School %>% 
...
write_csv(Child_512_School_demo_counts, here(
  paste0(
    'OUTPUT-FILES/CHILD/DESCRIPTIVES/Child-512-School-demo-counts-',
    format(Sys.Date(), "%Y-%m-%d"),
    '.csv'
  )
), 
