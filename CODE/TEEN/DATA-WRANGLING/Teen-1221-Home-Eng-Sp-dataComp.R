...
# Prep file to comp
Teen_1221_Home_scores <- Teen_1221_Home_items %>% 
  select(
    -(All_items_Teen_1221_Home), -(Gender:Region)
  )   
...
Teen_1221_Home_Sp <-
  suppressMessages(as_tibble(read_csv(
    here("INPUT-FILES/TEEN/SPM-2 Teen ages 1221 Home Report Questionnaire-Sp.csv")
  ))) %>% select(
    IDNumber,
    Age,
    # AgeGroup,
  ...
  filter(TOT_raw < 130)
# Save trimmed file 
write_csv(Teen_1221_Home_Sp, here(
  'INPUT-FILES/TEEN/SP-NORMS-INPUT/Teen-1221-Home-Sp-norms-input.csv'
))

# Prep file to comp
Teen_1221_Home_Sp_scores <- Teen_1221_Home_Sp %>% 
  select(
    -(All_items_Teen_1221_Home), -(Gender:Region)
  )   
...
Teen_1221_Home_comp <- bind_rows(Teen_1221_Home_scores, Teen_1221_Home_Sp_scores) 

# EXAMINE DATA---------------------------------

# Create frequency tables for TOT_raw by data source
Teen_1221_Home_comp_TOT_freq_data <- Teen_1221_Home_comp %>% group_by(data, age_range) %>% count(TOT_raw) %>%
  mutate(
    perc = round(100*(n/sum(n)), 4), 
    cum_per = round(100*(cumsum(n)/sum(n)), 4)
  )

# write_csv(Teen_1221_Home_comp_TOT_freq_data, here('OUTPUT-FILES/TEEN/FREQUENCIES/Teen-1221-Home-Eng-Sp-TOT-freq-data.csv'))


# Compute descriptive statistics, effect sizes for TOT_raw by data source
Teen_1221_Home_comp_TOT_desc_data <-
  Teen_1221_Home_comp %>% group_by(data) %>% arrange(data) %>% summarise(n = n(),
...
write_csv(Teen_1221_Home_comp_TOT_desc_data, here('OUTPUT-FILES/TEEN/DESCRIPTIVES/Teen-1221-Home-Eng-Sp-TOT-desc-dataSource.csv'))

# Generate single table comparing descriptives from Eng and Sp sources.
Teen_1221_Home_comp_TOT_desc_Eng <- Teen_1221_Home_comp_TOT_desc_data %>% 
  filter(data == 'Eng') %>% 
  setNames(., str_c(names(.), 'Eng', sep = '_')) %>% 
  ungroup() %>% 
  select(-matches('data|median|group'))
Teen_1221_Home_comp_TOT_desc_Sp <- Teen_1221_Home_comp_TOT_desc_data %>% 
  filter(data == 'Sp') %>% 
  setNames(., str_c(names(.), 'Sp', sep = '_')) %>% 
  ungroup() %>% 
  select(-matches('data|median|group'))

Teen_1221_Home_comp_TOT_desc_comp <- Teen_1221_Home_comp_TOT_desc_Eng %>% 
  bind_cols(Teen_1221_Home_comp_TOT_desc_Sp) %>% 
  # select(-age_range_Sp) %>% 
...
write_csv(Teen_1221_Home_comp_TOT_desc_comp, here('OUTPUT-FILES/TEEN/DESCRIPTIVES/Teen-1221-Home-Eng-Sp-TOT-desc-comp.csv'))

# Plot TOT_raw means, SDs by AgeGroup
mean_plot <- ggplot(data = Teen_1221_Home_comp_TOT_desc_data, aes(group, mean)) +
  geom_point(
    col = "blue",
    fill = "blue",
    alpha = .5,
  ...
  scale_x_continuous(breaks = seq(1, 2, 1), labels = unique(Teen_1221_Home_comp_TOT_desc_data$data)) +
