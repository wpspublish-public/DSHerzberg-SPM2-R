...
Teen_1221_Self <-
  suppressMessages(as_tibble(read_csv(
    here("INPUT-FILES/SPM-2 Teen ages 1221 Self-Report Questionnaire.csv")
  ))) %>% select(
    IDNumber,
    Age,
    AgeGroup,
...
write_csv(all_lookup, here(
  paste0(
    'OUTPUT-FILES/RAW-T-LOOKUP-TABLES/Teen-Self-raw-T-lookup-',
...
write_csv(all_lookup_pub, here(
  paste0(
    'OUTPUT-FILES/PRINT-FORMAT-NORMS-TABLES/Teen-Self-print-raw-T-lookup-',
    format(Sys.Date(), "%Y-%m-%d"),
    '.csv'
  )
))
