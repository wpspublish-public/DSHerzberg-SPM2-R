...
TOT_items_Child_512_School <- c("q0014", "q0015", "q0016", "q0017", "q0018", "q0019", "q0020", "q0021", "q0022", "q0023", 
...
Child_512_School <-
  suppressMessages(as_tibble(read_csv(
    here("DATA/SPM-2 Child ages 512 School Report Questionnaire.csv")
  ))) %>% select(
    IDNumber,
    Age,
    AgeGroup,
  ...
  mutate(TOT_raw = rowSums(.[TOT_items_Child_512_School])) %>% print()

# Create frequency tables for TOT_raw by AgeGroup
Child_512_School_TOT_freq_AgeGroup <- Child_512_School %>% group_by(AgeGroup) %>% count(TOT_raw) %>% 
...
Child_512_School_dup_IDnumber <- Child_512_School %>% count(IDNumber) %>% filter(n > 1)
write_csv(Child_512_School_dup_IDnumber, here("DATA/DATA_CLEANUP_FILES/Child_512_School_dup_IDnumber.csv"))

Child_512_School_missing_AgeGroup <- Child_512_School %>% filter(is.na(AgeGroup)) %>% select(IDNumber)
write_csv(Child_512_School_missing_AgeGroup, here("DATA/DATA_CLEANUP_FILES/Child_512_School_missing_AgeGroup.csv"))

# Regroup into two AgeGroups (AG2): 5-8 yr, 9-12 yr, evaluate these as norm strata.

# Create frequency tables for TOT_raw by AG2
Child_512_School_TOT_freq_AG2 <- Child_512_School %>% mutate(AG2 = case_when(
  Age <= 8 ~ "5-8 yr",
  Age >= 9 ~ "9-12 yr",
  TRUE ~ NA_character_
...
Child_512_School_TOT_desc_AG2 <-
  Child_512_School %>% mutate(AG2 = case_when(
    Age <= 8 ~ "5-8 yr",
    Age >= 9 ~ "9-12 yr",
    TRUE ~ NA_character_
