...
input_file_name <- "IT-1030-Home-allData-desamp"
...
demos_full <- map(
  ...
  ~
    sample_full %>%
    # group_by(AgeInMonths,!!sym(.x)) %>%
    group_by(!!sym(.x)) %>%
    summarize(n = n()) %>%
    pivot_wider(names_from = !!sym(.x), values_from = n)
) %>%
  # reduce(left_join, by = "AgeInMonths") %>%
  bind_cols() %>%
  # ungroup() %>%
  mutate(
  ...
  # relocate(c(sample, n), .before = "AgeInMonths")
  relocate(c(sample, n), .before = "Female")
