...
# Prep file to comp
Child_512_Home_scores <- Child_512_Home_items %>% 
  select(
    -(All_items_Child_512_Home), -(Gender:Region)
  )   
...
Child_512_Home_Sp <-
  suppressMessages(as_tibble(read_csv(
    here("INPUT-FILES/CHILD/SPM-2 Child ages 512 Home Report Questionnaire-Sp.csv")
  ))) %>% select(
    IDNumber,
    Age,
    # AgeGroup,
  ...
  filter(TOT_raw < 200)
# Save trimmed file 
write_csv(Child_512_Home_Sp, here(
  'INPUT-FILES/CHILD/SP-NORMS-INPUT/Child-512-Home-Sp-norms-input.csv'
))

# Prep file to comp
Child_512_Home_Sp_scores <- Child_512_Home_Sp %>% 
  select(
    -(All_items_Child_512_Home), -(Gender:Region)
  )   
...
Child_512_Home_comp <- bind_rows(Child_512_Home_scores, Child_512_Home_Sp_scores) 

# EXAMINE DATA---------------------------------

# Create frequency tables for TOT_raw by data source
Child_512_Home_comp_TOT_freq_data <- Child_512_Home_comp %>% group_by(data, age_range) %>% count(TOT_raw) %>%
  mutate(
    perc = round(100*(n/sum(n)), 4), 
    cum_per = round(100*(cumsum(n)/sum(n)), 4)
  )

# write_csv(Child_512_Home_comp_TOT_freq_data, here('OUTPUT-FILES/CHILD/FREQUENCIES/Child-512-Home-Eng-Sp-TOT-freq-data.csv'))


# Compute descriptive statistics, effect sizes for TOT_raw by data source
Child_512_Home_comp_TOT_desc_data <-
  Child_512_Home_comp %>% group_by(data) %>% arrange(data) %>% summarise(n = n(),
...
write_csv(Child_512_Home_comp_TOT_desc_data, here('OUTPUT-FILES/CHILD/DESCRIPTIVES/Child-512-Home-Eng-Sp-TOT-desc-dataSource.csv'))

# Generate single table comparing descriptives from Eng and Sp sources.
Child_512_Home_comp_TOT_desc_Eng <- Child_512_Home_comp_TOT_desc_data %>% 
  filter(data == 'Eng') %>% 
  setNames(., str_c(names(.), 'Eng', sep = '_')) %>% 
  ungroup() %>% 
  select(-matches('data|median|group'))
Child_512_Home_comp_TOT_desc_Sp <- Child_512_Home_comp_TOT_desc_data %>% 
  filter(data == 'Sp') %>% 
  setNames(., str_c(names(.), 'Sp', sep = '_')) %>% 
  ungroup() %>% 
  select(-matches('data|median|group'))

Child_512_Home_comp_TOT_desc_comp <- Child_512_Home_comp_TOT_desc_Eng %>% 
  bind_cols(Child_512_Home_comp_TOT_desc_Sp) %>% 
  # select(-age_range_Sp) %>% 
...
write_csv(Child_512_Home_comp_TOT_desc_comp, here('OUTPUT-FILES/CHILD/DESCRIPTIVES/Child-512-Home-Eng-Sp-TOT-desc-comp.csv'))

# Plot TOT_raw means, SDs by AgeGroup
mean_plot <- ggplot(data = Child_512_Home_comp_TOT_desc_data, aes(group, mean)) +
  geom_point(
    col = "blue",
    fill = "blue",
    alpha = .5,
  ...
  scale_x_continuous(breaks = seq(1, 2, 1), labels = unique(Child_512_Home_comp_TOT_desc_data$data)) +
