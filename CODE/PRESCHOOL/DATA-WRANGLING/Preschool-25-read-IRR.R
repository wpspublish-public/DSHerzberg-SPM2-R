...
Preschool_25_Home_IRR <-
  suppressMessages(as_tibble(read_csv(
    here("INPUT-FILES/PRESCHOOL/FORM-B/SPM-2 Preschooler ages 25 Home Report Questionnaire B.csv")
  ))) %>% select(
    IDNumber,
    Age,
    Gender,
...
Preschool_24_Home_IRR <- Preschool_25_Home_IRR %>% filter(age_range == "2 to 4 years")
Preschool_5_Home_IRR <- Preschool_25_Home_IRR %>% filter(age_range == "5 years")
...
output_24 <- map_dfc(score_names,
                     ~
                       Preschool_24_Home_IRR %>% left_join(eval(as.name(
                         str_c(.x, '_24_lookup_col')
                       )),
                       by = str_c(.x, '_raw')) %>%
                       select(!!str_c(.x, '_NT'))) %>%
  bind_cols(Preschool_24_Home_IRR, .) %>% 
  arrange(IDNumber)
...
write_csv(
  output_24,
  here(
    'OUTPUT-FILES/PRESCHOOL/T-SCORES-PER-CASE/Preschool-24-Home-IRR-T-Scores-per-case.csv'
  ),
  na = ''
)
...
output_5 <- map_dfc(score_names,
                     ~
                       Preschool_5_Home_IRR %>% left_join(eval(as.name(
                         str_c(.x, '_5_lookup_col')
                       )),
                       by = str_c(.x, '_raw')) %>%
                       select(!!str_c(.x, '_NT'))) %>%
  bind_cols(Preschool_5_Home_IRR, .) %>% 
  arrange(IDNumber)
...
write_csv(
  output_5,
  here(
    'OUTPUT-FILES/PRESCHOOL/T-SCORES-PER-CASE/Preschool-5-Home-IRR-T-Scores-per-case.csv'
  ),
  na = ''
)
...
Preschool_25_School_IRR <-
  suppressMessages(as_tibble(read_csv(
    here("INPUT-FILES/PRESCHOOL/FORM-B/SPM-2 Preschooler ages 25 School Report Questionnaire B.csv")
  ))) %>% select(
    IDNumber,
    Age,
    Gender,
...
Preschool_24_School_IRR <- Preschool_25_School_IRR %>% filter(age_range == "2 to 4 years")
Preschool_5_School_IRR <- Preschool_25_School_IRR %>% filter(age_range == "5 years")
...
output_24 <- map_dfc(score_names,
                     ~
                       Preschool_24_School_IRR %>% left_join(eval(as.name(
                         str_c(.x, '_24_lookup_col')
                       )),
                       by = str_c(.x, '_raw')) %>%
                       select(!!str_c(.x, '_NT'))) %>%
  bind_cols(Preschool_24_School_IRR, .) %>% 
  arrange(IDNumber)
...
write_csv(
  output_24,
  here(
    'OUTPUT-FILES/PRESCHOOL/T-SCORES-PER-CASE/Preschool-24-School-IRR-T-Scores-per-case.csv'
  ),
  na = ''
)
...
output_5 <- map_dfc(score_names,
                    ~
                      Preschool_5_School_IRR %>% left_join(eval(as.name(
                        str_c(.x, '_5_lookup_col')
                      )),
                      by = str_c(.x, '_raw')) %>%
                      select(!!str_c(.x, '_NT'))) %>%
  bind_cols(Preschool_5_School_IRR, .) %>% 
  arrange(IDNumber)
...
write_csv(
  output_5,
  here(
    'OUTPUT-FILES/PRESCHOOL/T-SCORES-PER-CASE/Preschool-5-School-IRR-T-Scores-per-case.csv'
  ),
  na = ''
)
