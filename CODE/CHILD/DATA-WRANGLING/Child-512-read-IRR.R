...
Child_512_Home_IRR <-
  suppressMessages(as_tibble(read_csv(
    here("INPUT-FILES/CHILD/FORM-B/SPM-2 Child ages 512 Home Report Questionnaire B.csv")
  ))) %>% select(
    IDNumber,
    Age,
    Gender,
...
output_512 <- map_dfc(score_names,
                     ~
                       Child_512_Home_IRR %>% left_join(eval(as.name(
                         str_c(.x, '_512_lookup_col')
                       )),
                       by = str_c(.x, '_raw')) %>%
                       select(!!str_c(.x, '_NT'))) %>%
  bind_cols(Child_512_Home_IRR, .) %>% 
  arrange(IDNumber)
...
write_csv(
  output_512,
  here(
    'OUTPUT-FILES/CHILD/T-SCORES-PER-CASE/Child-512-Home-IRR-T-Scores-per-case.csv'
  ),
  na = ''
)
...
Child_512_School_IRR <-
  suppressMessages(as_tibble(read_csv(
    here("INPUT-FILES/CHILD/FORM-B/SPM-2 Child ages 512 School Report Questionnaire B.csv")
  ))) %>% select(
    IDNumber,
    Age,
    Gender,
...
output_512 <- map_dfc(score_names,
                       ~
                         Child_512_School_IRR %>% left_join(eval(as.name(
                           str_c(.x, '_512_lookup_col')
                         )),
                         by = str_c(.x, '_raw')) %>%
                         select(!!str_c(.x, '_NT'))) %>%
...
write_csv(
  output_512,
  here(
    'OUTPUT-FILES/CHILD/T-SCORES-PER-CASE/Child-512-School-IRR-T-Scores-per-case.csv'
  ),
  na = ''
)
