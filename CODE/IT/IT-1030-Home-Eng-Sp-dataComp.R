...
# Prep file to comp
IT_1030_Home_scores <- IT_1030_Home_items %>% 
  select(
    -(All_items_IT_1030_Home), -(Gender:Region)
  )   
...
IT_1030_Home_Sp <-
  suppressMessages(as_tibble(read_csv(
    here("INPUT-FILES/IT/SPM-2 InfantToddler 1030 Months-Sp.csv")
  ))) %>% select(
    IDNumber,
    AgeInMonths,
    # AgeGroup,
  ...
  filter(TOT_raw < 200)
# Save trimmed file 
write_csv(IT_1030_Home_Sp, here(
  'INPUT-FILES/IT/SP-NORMS-INPUT/IT-1030-Home-Sp-norms-input.csv'
))

# Prep file to comp
IT_1030_Home_Sp_scores <- IT_1030_Home_Sp %>% 
  select(
    -(All_items_IT_1030_Home), -(Gender:Region)
  )   
...
IT_1030_Home_comp <- bind_rows(IT_1030_Home_scores, IT_1030_Home_Sp_scores) 

# EXAMINE DATA---------------------------------

# Create frequency tables for TOT_raw by data source
IT_1030_Home_comp_TOT_freq_data <- IT_1030_Home_comp %>% group_by(data, age_range) %>% count(TOT_raw) %>%
  mutate(
    perc = round(100*(n/sum(n)), 4), 
    cum_per = round(100*(cumsum(n)/sum(n)), 4)
  )

# write_csv(IT_1030_Home_comp_TOT_freq_data, here('OUTPUT-FILES/IT/FREQUENCIES/IT-1030-Home-Eng-Sp-TOT-freq-data.csv'))


# Compute descriptive statistics, effect sizes for TOT_raw by data source
IT_1030_Home_comp_TOT_desc_data <-
  IT_1030_Home_comp %>% group_by(data) %>% arrange(data) %>% summarise(n = n(),
...
write_csv(IT_1030_Home_comp_TOT_desc_data, here('OUTPUT-FILES/IT/DESCRIPTIVES/IT-1030-Home-Eng-Sp-TOT-desc-dataSource.csv'))

# Generate single table comparing descriptives from Eng and Sp sources.
IT_1030_Home_comp_TOT_desc_Eng <- IT_1030_Home_comp_TOT_desc_data %>% 
  filter(data == 'Eng') %>% 
  setNames(., str_c(names(.), 'Eng', sep = '_')) %>% 
  ungroup() %>% 
  select(-matches('data|median|group'))
IT_1030_Home_comp_TOT_desc_Sp <- IT_1030_Home_comp_TOT_desc_data %>% 
  filter(data == 'Sp') %>% 
  setNames(., str_c(names(.), 'Sp', sep = '_')) %>% 
  ungroup() %>% 
  select(-matches('data|median|group'))

IT_1030_Home_comp_TOT_desc_comp <- IT_1030_Home_comp_TOT_desc_Eng %>% 
  bind_cols(IT_1030_Home_comp_TOT_desc_Sp) %>% 
  # select(-age_range_Sp) %>% 
...
write_csv(IT_1030_Home_comp_TOT_desc_comp, here('OUTPUT-FILES/IT/DESCRIPTIVES/IT-1030-Home-Eng-Sp-TOT-desc-comp.csv'))

# Plot TOT_raw means, SDs by AgeGroup
mean_plot <- ggplot(data = IT_1030_Home_comp_TOT_desc_data, aes(group, mean)) +
  geom_point(
    col = "blue",
    fill = "blue",
    alpha = .5,
  ...
  scale_x_continuous(breaks = seq(1, 2, 1), labels = unique(IT_1030_Home_comp_TOT_desc_data$data)) +
