...
IT_49_Home_IRR <-
  suppressMessages(as_tibble(read_csv(
    here("INPUT-FILES/IT/FORM-B/SPM-2 InfantToddler 49 Months B.csv")
  ))) %>% select(
    IDNumber,
    AgeInMonths,
    Gender,
...
IT_46_Home_IRR <- IT_49_Home_IRR %>% filter(age_range == "03.5 to 6 mo")
IT_79_Home_IRR <- IT_49_Home_IRR %>% filter(age_range == "07 to 10.5 mo")
...
output_46 <- map_dfc(score_names,
                     ~
                       IT_46_Home_IRR %>% left_join(eval(as.name(
                         str_c(.x, '_46_lookup_col')
                       )),
                       by = str_c(.x, '_raw')) %>%
                       select(!!str_c(.x, '_NT'))) %>%
  bind_cols(IT_46_Home_IRR, .) %>% 
  arrange(IDNumber)
...
write_csv(
  output_46,
  here(
    'OUTPUT-FILES/IT/T-SCORES-PER-CASE/IT-46-Home-IRR-T-Scores-per-case.csv'
  )
)
...
output_79 <- map_dfc(score_names,
                     ~
                       IT_79_Home_IRR %>% left_join(eval(as.name(
                         str_c(.x, '_79_lookup_col')
                       )),
                       by = str_c(.x, '_raw')) %>%
                       select(!!str_c(.x, '_NT'))) %>%
  bind_cols(IT_79_Home_IRR, .) %>% 
  arrange(IDNumber)
...
write_csv(
  output_79,
  here(
    'OUTPUT-FILES/IT/T-SCORES-PER-CASE/IT-79-Home-IRR-T-Scores-per-case.csv'
  )
)
...
IT_1030_Home_IRR <-
  suppressMessages(as_tibble(read_csv(
    here("INPUT-FILES/IT/FORM-B/SPM-2 InfantToddler 1030 Months B.csv")
  ))) %>% select(
    IDNumber,
    AgeInMonths,
    Gender,
...
IT_1020_Home_IRR <- IT_1030_Home_IRR %>% filter(age_range == "09.5 to 20 mo")
IT_2130_Home_IRR <- IT_1030_Home_IRR %>% filter(age_range == "21 to 31.5 mo")
...
output_1020 <- map_dfc(score_names,
                       ~
                         IT_1020_Home_IRR %>% left_join(eval(as.name(
                           str_c(.x, '_1020_lookup_col')
                         )),
                         by = str_c(.x, '_raw')) %>%
                         select(!!str_c(.x, '_NT'))) %>%
  bind_cols(IT_1020_Home_IRR, .) %>% 
  arrange(IDNumber)
...
write_csv(
  output_1020,
  here(
    'OUTPUT-FILES/IT/T-SCORES-PER-CASE/IT-1020-Home-IRR-T-Scores-per-case.csv'
  )
)
...
output_2130 <- map_dfc(score_names,
                       ~
                         IT_2130_Home_IRR %>% left_join(eval(as.name(
                           str_c(.x, '_2130_lookup_col')
                         )),
                         by = str_c(.x, '_raw')) %>%
                         select(!!str_c(.x, '_NT'))) %>%
  bind_cols(IT_2130_Home_IRR, .) %>% 
  arrange(IDNumber)
...
write_csv(
  output_2130,
  here(
    'OUTPUT-FILES/IT/T-SCORES-PER-CASE/IT-2130-Home-IRR-T-Scores-per-case.csv'
  )
)
...
IT_Caregiver_IRR <-
  suppressMessages(as_tibble(read_csv(
    here("INPUT-FILES/IT/FORM-B/SPM2_InfantToddler_Caregiver_Combo B.csv")
  ))) %>% select(
    IDNumber,
    AgeInMonths,
    Gender,
...
output_Caregiver <- map_dfc(score_names,
                       ~
                         IT_Caregiver_IRR %>% left_join(eval(as.name(
                           str_c(.x, '_Caregiver_lookup_col')
                         )),
                         by = str_c(.x, '_raw')) %>%
                         select(!!str_c(.x, '_NT'))) %>%
  bind_cols(IT_Caregiver_IRR, .) %>% 
  arrange(IDNumber)
...
write_csv(
  output_Caregiver,
  here(
    'OUTPUT-FILES/IT/T-SCORES-PER-CASE/IT-Caregiver-IRR-T-Scores-per-case.csv'
  ), 
  na = ''
)
