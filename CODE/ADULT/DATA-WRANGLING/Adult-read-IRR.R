...
Adult_Other_IRR <-
  suppressMessages(as_tibble(read_csv(
    here("INPUT-FILES/ADULT/FORM-B/SPM-2 Adult ages 1690 Other Report Questionnaire B.csv")
  ))) %>% select(
    IDNumber,
    Age,
    Gender,
...
output_1690 <- map_dfc(score_names,
                       ~
                         Adult_Other_IRR %>% left_join(eval(as.name(
                           str_c(.x, '_1690_lookup_col')
                         )),
                         by = str_c(.x, '_raw')) %>%
                         select(!!str_c(.x, '_NT'))) %>%
  bind_cols(Adult_Other_IRR, .) %>% 
  arrange(IDNumber)
...
write_csv(
  output_1690,
  here(
    'OUTPUT-FILES/ADULT/T-SCORES-PER-CASE/Adult-Other-IRR-T-Scores-per-case.csv'
  ),
  na = ''
)
