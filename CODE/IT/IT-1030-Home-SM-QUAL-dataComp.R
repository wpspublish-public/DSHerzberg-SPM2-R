...
IT_1030_Home_items <-
  suppressMessages(as_tibble(read_csv(
    here("INPUT-FILES/IT/SPM-2 SM Qual COMBO InfantToddler 1030 Months.csv")
  ))) %>% select(
    IDNumber,
    AgeInMonths,
    # AgeGroup,
...
rm(list = ls(pattern='.*items_IT_1030_Home'))

IT_1030_Home <- IT_1030_Home_items %>% 
  select(
    -(q0011:q0125)
  )   
...
IT_1030_Home_TOT_freq_AgeGroup <- IT_1030_Home %>% group_by(data, age_range) %>% count(TOT_raw) %>%
  mutate(
    perc = round(100*(n/sum(n)), 4), 
    cum_per = round(100*(cumsum(n)/sum(n)), 4)
  )

# write_csv(IT_1030_Home_TOT_freq_AgeGroup, here('OUTPUT-FILES/IT/FREQUENCIES/IT-1030-Home-TOT-freq-AgeGroup.csv'))


# Compute descriptive statistics, effect sizes for TOT_raw by AgeGroup
IT_1030_Home_TOT_desc_AgeGroup_data <-
  IT_1030_Home %>% group_by(data, age_range) %>% arrange(data, age_range) %>% summarise(n = n(),
...
IT_1030_Home_TOT_desc_AgeGroup <-
  IT_1030_Home %>% group_by(age_range) %>% arrange(age_range) %>% summarise(n = n(),
                                                                                          median = round(median(TOT_raw), 2),
                                                                                          mean = round(mean(TOT_raw), 2),
                                                                                          sd = round(sd(TOT_raw), 2)) %>%
  mutate(ES = round((mean - lag(mean))/((sd + lag(sd))/2),2),
...
write_csv(IT_1030_Home_TOT_desc_AgeGroup_data, here('OUTPUT-FILES/IT/DESCRIPTIVES/IT-1030-Home-TOT-desc-dataSource.csv'))
write_csv(IT_1030_Home_TOT_desc_AgeGroup, here('OUTPUT-FILES/IT/DESCRIPTIVES/IT-1030-Home-TOT-desc.csv'))


# Generate single table comparing descriptives from SM and Qual sources.
IT_1030_Home_TOT_desc_SM <- IT_1030_Home_TOT_desc_AgeGroup_data %>% 
  filter(data == 'SM') %>% 
  setNames(., str_c(names(.), 'SM', sep = '_')) %>% 
  ungroup() %>% 
  select(-matches('data|median|group'))
IT_1030_Home_TOT_desc_Qual <- IT_1030_Home_TOT_desc_AgeGroup_data %>% 
  filter(data == 'Qual') %>% 
  setNames(., str_c(names(.), 'Qual', sep = '_')) %>% 
  ungroup() %>% 
  select(-matches('data|median|group'))

IT_1030_Home_TOT_desc_comp <- IT_1030_Home_TOT_desc_SM %>% 
  bind_cols(IT_1030_Home_TOT_desc_Qual) %>% 
...
write_csv(IT_1030_Home_TOT_desc_comp, here('OUTPUT-FILES/IT/DESCRIPTIVES/IT-1030-Home-TOT-desc-comp.csv'))

# Plot TOT_raw means, SDs by AgeGroup
mean_plot <- ggplot(data = IT_1030_Home_TOT_desc_AgeGroup_data, aes(group, mean)) +
  geom_point(
    col = "blue",
    fill = "blue",
    alpha = .5,
  ...
  scale_x_continuous(breaks = seq(1, 2, 1), labels = unique(IT_1030_Home_TOT_desc_AgeGroup_data$age_range)) +
