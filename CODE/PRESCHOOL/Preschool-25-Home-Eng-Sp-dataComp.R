...
# Prep file to comp
Preschool_25_Home_scores <- Preschool_25_Home_items %>% 
  select(
    -(All_items_Preschool_25_Home), -(Gender:Region)
  )   
...
Preschool_25_Home_Sp <-
  suppressMessages(as_tibble(read_csv(
    here("INPUT-FILES/PRESCHOOL/SPM-2 Preschooler ages 25 Home Report Questionnaire-Sp.csv")
  ))) %>% select(
    IDNumber,
    Age,
    # AgeGroup,
...
Preschool_25_Home_comp <- bind_rows(Preschool_25_Home_scores, Preschool_25_Home_Sp_scores) 

# EXAMINE DATA---------------------------------

# Create frequency tables for TOT_raw by data source
Preschool_25_Home_comp_TOT_freq_data <- Preschool_25_Home_comp %>% group_by(data, age_range) %>% count(TOT_raw) %>%
  mutate(
    perc = round(100*(n/sum(n)), 4), 
    cum_per = round(100*(cumsum(n)/sum(n)), 4)
  )

# write_csv(Preschool_25_Home_comp_TOT_freq_data, here('OUTPUT-FILES/PRESCHOOL/FREQUENCIES/Preschool-25-Home-Eng-Sp-TOT-freq-data.csv'))


# Compute descriptive statistics, effect sizes for TOT_raw by data source
Preschool_25_Home_comp_TOT_desc_data <-
  Preschool_25_Home_comp %>% group_by(data) %>% arrange(data) %>% summarise(n = n(),
...
write_csv(Preschool_25_Home_comp_TOT_desc_data, here('OUTPUT-FILES/PRESCHOOL/DESCRIPTIVES/Preschool-25-Home-Eng-Sp-TOT-desc-dataSource.csv'))

# Generate single table comparing descriptives from Eng and Sp sources.
Preschool_25_Home_comp_TOT_desc_Eng <- Preschool_25_Home_comp_TOT_desc_data %>% 
  filter(data == 'Eng') %>% 
  setNames(., str_c(names(.), 'Eng', sep = '_')) %>% 
  ungroup() %>% 
  select(-matches('data|median|group'))
Preschool_25_Home_comp_TOT_desc_Sp <- Preschool_25_Home_comp_TOT_desc_data %>% 
  filter(data == 'Sp') %>% 
  setNames(., str_c(names(.), 'Sp', sep = '_')) %>% 
  ungroup() %>% 
  select(-matches('data|median|group'))

Preschool_25_Home_comp_TOT_desc_comp <- Preschool_25_Home_comp_TOT_desc_Eng %>% 
  bind_cols(Preschool_25_Home_comp_TOT_desc_Sp) %>% 
  # select(-age_range_Sp) %>% 
...
write_csv(Preschool_25_Home_comp_TOT_desc_comp, here('OUTPUT-FILES/PRESCHOOL/DESCRIPTIVES/Preschool-25-Home-Eng-Sp-TOT-desc-comp.csv'))

# Plot TOT_raw means, SDs by AgeGroup
mean_plot <- ggplot(data = Preschool_25_Home_comp_TOT_desc_data, aes(group, mean)) +
  geom_point(
    col = "blue",
    fill = "blue",
    alpha = .5,
  ...
  scale_x_continuous(breaks = seq(1, 2, 1), labels = unique(Preschool_25_Home_comp_TOT_desc_data$data)) +
