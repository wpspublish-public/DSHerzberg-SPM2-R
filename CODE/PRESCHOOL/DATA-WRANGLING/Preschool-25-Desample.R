...
Preschool_25_Home_desamp1 <- bind_rows(Preschool_25_Home_White_BAplus, Preschool_25_Home_not_White_BAplus) %>% 
  arrange(IDNumber)
...
anyDuplicated(Preschool_25_Home_desamp$IDNumber)

# Check for any NAs on IDNumber, returns TRUE if NA exist
any(is.na(Preschool_25_Home_desamp$IDNumber))

# extract cases with Dup ID numbers or NA on IDNumber, write out for investigation
Preschool_25_Home_dupMissIDs <- Preschool_25_Home_desamp %>% 
  ...
  write_csv(
    here(
      paste0(
        'OUTPUT-FILES/PRESCHOOL/DUP-IDS/Preschool-25-Home-dupIDs-missingIDs-',
...
# Check for any dupIDs (anyDuplicated() returns row number of FIRST dup ID encountered)
anyDuplicated(Preschool_25_School$IDNumber)

# Check for any NAs on IDNumber, returns TRUE if NA exist
any(is.na(Preschool_25_School$IDNumber))

# extract cases with Dup ID numbers or NA on IDNumber, write out for investigation
# Preschool_25_School_dupMissIDs <- Preschool_25_School_desamp %>% 
#   mutate(dup = duplicated(IDNumber)) %>% 
#   filter(dup == TRUE | is.na(IDNumber)) %>% 
#   select(-dup) %>% 
#   write_csv(
#     here(
#       paste0(
#         'OUTPUT-FILES/PRESCHOOL/DUP-IDS/Preschool-25-School-dupIDs-missingIDs-',
#         format(Sys.Date(), "%Y-%m-%d"),
#         '.csv'
#       )
#     ), 
