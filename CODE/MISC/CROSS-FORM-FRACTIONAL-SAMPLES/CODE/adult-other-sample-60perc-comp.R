...
input_file_name <- "Adult-Other-allData-desamp"
...
demos_full <- map(
  c("Gender", "HighestEducation", "Ethnicity", "Region"),
  ~
    sample_full %>%
    group_by(age_range,!!sym(.x)) %>%
    summarize(n = n()) %>%
    pivot_wider(names_from = !!sym(.x), values_from = n)
) %>%
  reduce(left_join, by = "age_range") %>%
  ungroup() %>%
  mutate(
  ...
  relocate(c(sample, n), .before = "age_range")
